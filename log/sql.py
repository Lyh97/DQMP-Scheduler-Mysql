...
query = {
    'select_tasklist': '''
      ...
      From task
      WHERE remove = 0 AND upload_user_id = ?
    ...
    'SelectDailyErrorList':'''
      ...
      from dailylog
      WHERE result_time = ? AND status = 'Fail') AND upload_user_id = ?
    ''',
    ...
    'SelectSpeDailyErrorList':'''
      ...
      WHERE taskid in (SELECT taskid 
      from dailylog
      WHERE result_time = ? AND status = 'Fail' AND category = ?) AND upload_user_id = ?
    ''',
